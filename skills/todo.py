...
class Todo():
    ...
    def show(cls):
        ...
        if len(cls.__todos) == 0:
            ...
            for item in cls.__todos:
                print(count, item.title, item.status,
                      item.priority, item.age, item.id)
                count += 1
            print("")
    ...
    def remove_item(self, uuid: str = None, title: str = None) -> bool:
        if title is None and uuid is None:
            print(
                "You need to provide some detail for me to remove an item from the list")
...
class TodoSkill():
    ...
    def add_todo(self,glados: AI) -> bool:
        item = Item()
        msg = self.msg_list[4]
        glados.say(msg, wav_name=wav_name(self, msg))
        try:
    ...
    def remove_todo(self,glados: AI) -> bool:
        msg = self.msg_list[0]
        glados.say(msg, wav_name=wav_name(self, msg))
        try:
